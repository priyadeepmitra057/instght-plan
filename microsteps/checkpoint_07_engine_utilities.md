# CHECKPOINT 07: Engine Utilities (Passion Utils, Candidate, Subcategory)

Directly modified:   passion_utils.py, candidate.py, marketplace_subcategory.py
Indirectly affected: Passion detection
Code blocks used:    CB-P3-02, CB-P3-03, CB-P3-04
Risk:                MEDIUM
Depends on:          CHECKPOINT 06
...
CONTEXT
Implementing shared utilities, candidate containers, and marketplace subcategory resolution logic.
...
STEPS
  ...
  STEP [7.1]
  File:           passion_utils.py
  Action:         CREATE
  Source file:    passion_plan_part3.md
  Source section: 12. passion_utils.py
  Block ID:       CB-P3-02
  Flags:          NONE
  ...
  Instruction: Create `passion_utils.py` with verbatim content.
...
def safe_numeric(val: Any, default: float = 0.0, *, raise_on_invalid: bool = False) -> float:
  ...
  Rollback: Delete passion_utils.py.
...
class Candidate:
  ...
  Action:         CREATE
  Source file:    passion_plan_part3.md
  Source section: 14. marketplace_subcategory.py
  Block ID:       CB-P3-04
  Flags:          NONE
  ...
  Instruction: Create `marketplace_subcategory.py` with verbatim content.
...
def enrich_subcategories(df: pd.DataFrame) -> pd.DataFrame:
  ...
  Rollback: Delete marketplace_subcategory.py.

POST-EXECUTION VALIDATION
[ ] `passion_utils.py` exists.
[ ] `candidate.py` contains `Candidate.passion` and `Candidate.__lt__`.
[ ] `marketplace_subcategory.py` exists.
[ ] `python3 -m py_compile passion_utils.py candidate.py marketplace_subcategory.py` succeeds.

GO / NO-GO
All checks pass → proceed to CHECKPOINT [08]
